...
Uses a closure factory pattern — create_memory_tools(db_path, model) returns
the three tools with db_path and model captured, eliminating module-level globals.
"""
...
def create_memory_tools(db_path: str, model: object | None = None) -> list[BaseTool]:
    """Create the three memory tools with db_path and model captured in closures.

    Returns [list_sessions, recall_session, summarize_session].
    """
    ...
    def list_sessions() -> str:
        ...
        当用户询问"我们之前聊过什么"时使用。
        """
        sessions = list_thread_ids(db_path)
        if not sessions:
            ssummaries = list_summaries(db_path)
            if not ssummaries:
                return "暂无历史会话记录。"
            sessions = ssummaries
        ...
        for s in sessions:
            ...
            summary = get_summary(db_path, tid)
    ...
    def recall_session(session_id: str, summary_only: bool = True) -> str:
        ...
        """
        if summary_only:
            summary = get_summary(db_path, session_id)
            if summary is None:
    ...
    def summarize_session(session_id: str = "") -> str:
        ...
        title, summary = generate_session_summary(text, model)
        save_summary(db_path, session_id, title, summary, len(messages))
