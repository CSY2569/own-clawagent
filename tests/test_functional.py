...
def agent(settings):
    ...
    from clawagent.tools.memory_tools import configure as _cfg
    # Minimal setup: create a memory db so tools can function
    _cfg(str(settings.memory_db_path))
    ...
    graph = create_react_agent(
        ...
        checkpointer=SqliteSaver(conn), prompt=prompt,
    )
...
class TestAgentSetup:
    def test_all_tools_defined(self):
        ...
        assert names == {
            ...
            "list_sessions", "recall_session", "summarize_session",
        }
...
class TestWriteFileTool:
    ...
    def test_write_outside_project(self):
        with pytest.raises(ValueError, match="outside the project"):
            write_file.invoke({"path": "/tmp/evil.txt", "content": "bad"})
...
class TestRunCommandTool:
    ...
    def test_failing_command(self):
        result = run_command.invoke({"command": "exit 42"})
        assert "42" in result
...
class TestGreetTool:
    def test_greet_basic(self):
        result = next(t for t in ALL_TOOLS if t.name == "greet").invoke({"name": "Alice"})
        assert "Hello" in result
        assert "Alice" in result

    def test_greet_empty_name(self):
        result = next(t for t in ALL_TOOLS if t.name == "greet").invoke({"name": ""})
        assert "Hello" in result
