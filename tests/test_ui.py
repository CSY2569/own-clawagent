...
class TestConversationStats:
    ...
    def test_cost_with_cache_hit(self) -> None:
        cs = ConversationStats()
        cs.update(Usage(prompt_cache_hit_tokens=500_000))
        pricing = PriceConfig(input_per_1m=1.0, cache_hit_per_1m=0.02, output_per_1m=2.0)
        expected = 0.01  # 500K cache hit * 0.02/1M
        assert cs.cost(pricing) == expected

    def test_cost_deepseek_cache_comprehensive(self) -> None:
        cs = ConversationStats()
        cs.update(Usage(
        ...
        pricing = PriceConfig(input_per_1m=1.0, cache_hit_per_1m=0.02, output_per_1m=2.0)
        expected = 1.0 + 0.008 + 1.0  # 1M input + 400K cache hit + 500K output
        assert cs.cost(pricing) == expected
